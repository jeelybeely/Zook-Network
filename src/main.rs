...
use api::bridge_api::bridge_routes;
use api::governance_api::governance_routes;
...
async fn main() {
    ...
    let sender = "SP3K2K92ZXAN5DZ96FEAB5A6NNG2MX74E9KPW0XJ7"; // Replace with actual Principal
    ...
    let validator_rewards = Arc::new(ValidatorRewards::new(
        100,
        PathBuf::from("validator_rewards.json"),
    ));

    // Initialize additional governance components
    ...
    // API keys and rate limiter for governance API
    let api_keys = Arc::new(HashMap::new()); // Replace with actual API keys if needed
    let rate_limiter = Arc::new(crate::api::security::RateLimiter::new(100, std::time::Duration::from_secs(60)));

    // Initialize APIs
    let governance_api = governance_routes(
        cross_layer_governance.clone(),
        api_keys.clone(),
        rate_limiter.clone(),
    );
    let bridge_api = bridge_routes(
        HashMap::new().keys().cloned().collect(),
        bridge_ledger.clone(),
    );
