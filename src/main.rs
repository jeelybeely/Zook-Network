...
use governance::validator_rewards::ValidatorRewards; // Fixed import
...
async fn main() {
    ...
    // Fixed: Add compliance_threshold (e.g., 70) as the third argument
    ...
    // Fixed: Instantiate ValidatorRewards and pass into CrossLayerGovernance
    let validator_rewards = Arc::new(ValidatorRewards::new(100, PathBuf::from("validator_rewards.json")));
    ...
    let cross_layer_governance = Arc::new(CrossLayerGovernance::new(
        ...
        validator_rewards.clone(), // Pass ValidatorRewards here
        governance_storage_path,
    ));

    // API keys and rate limiter
    ...
    let bridge_api = BridgeAPI::new(HashSet::new(), bridge_ledger.clone());
