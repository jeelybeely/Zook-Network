...
impl CrossLayerSync {
    ...
    pub fn anchor_state(
        ...
        let states = match self.anchored_states.lock() {
            Ok(mut guard) => {
                ...
                guard.push(anchored_state);
                guard.clone()
            }
            Err(_) => return Err("Mutex lock failed".to_string()),
        };
    ...
    pub fn validate_state(&self, state_root: &str) -> bool {
        match self.anchored_states.lock() {
            Ok(states) => states.iter().any(|state| state.state_root == state_root),
            Err(_) => false, // Fallback if lock fails
        }
    }

    pub fn get_latest_state(&self) -> Option<AnchoredState> {
        match self.anchored_states.lock() {
            Ok(states) => states.last().cloned(),
            Err(_) => None, // Return None if lock fails
        }
    }
