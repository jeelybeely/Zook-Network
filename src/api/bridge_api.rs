    ...
    let mint = warp::post()
        ...
        .and_then(
            |request: MintRequest,
             auth: ApiKey,
             _rate_limit: (),
             ledger: Arc<Mutex<BridgeLedger>>| async move { mint_handler(request, auth, ledger).await },
        );
    ...
    let burn = warp::post()
        ...
        .and_then(
            |request: BurnRequest,
             auth: ApiKey,
             _rate_limit: (),
             ledger: Arc<Mutex<BridgeLedger>>| async move { burn_handler(request, auth, ledger).await },
        );

    warp::any().and(mint.or(burn))
}
