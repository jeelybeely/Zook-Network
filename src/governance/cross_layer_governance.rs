...
use crate::bridge::state_anchoring::generate_state_summary;
...
impl CrossLayerGovernance {
    ...
    pub fn generate_anchor_summary(&self, block_height: u64, total_transactions: u64, merkle_tree: &MerkleTree) -> Result<L2StateSummary, String> {
        let summary = generate_state_summary(merkle_tree, block_height, total_transactions);
        self.state_anchoring.anchor_state(summary.clone())?;
        println!("Generated and anchored state summary: {:?}", summary);
        Ok(summary)
    }
}
