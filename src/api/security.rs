...
pub struct ApiKey {
    pub key: String,
    pub permissions: Vec<String>, // Changed from HashSet to Vec for compatibility
}
...
/// Auth filter with proper tuple wrapping
pub fn with_auth(
            ...
            async move {
                match auth {
                    Some(token) if keys.contains_key(&token) => {
                        let api_key = keys.get(&token).unwrap().clone();
                        Ok((api_key,)) // Properly wrap in a tuple
                    }
                    _ => Err(warp::reject::custom(Unauthorized)),
                }
            }
        })
        .untuple_one() // Ensure the tuple is flattened
}

/// Rate limiter filter remains unchanged
