...
pub fn governance_routes(
    ...
    let submit_proposal = warp::post()
        ...
        .and_then(
            |request: ProposalRequest, auth: ApiKey, governance: Arc<CrossLayerGovernance>| async move {
                submit_proposal_handler(request, auth, governance).await
            },
        );
    ...
    let list_proposals = warp::get()
        ...
        .and(with_governance(governance.clone()))
        .and_then(
            |governance: Arc<CrossLayerGovernance>| async move {
                list_proposals_handler(governance).await
            },
        );
